...
bmargin = 20
r = 100
arm_length = int(r * 0.8)
circle_thickness = 4
arm_thickness = 15
circle_color = (192,192,192)
arm_color = (45,45,255)

Canvas_W = lmargin + rmargin + r * 4 + margin
...
t =10
for n in range(10):
    clocks.animation_start(tcanvas, n, t)
    for i in range(int(t * FPS)):
        tcanvas = canvas.copy()
        clocks.animation_next(tcanvas)
        cv2.imshow("digit [%d]"%(n), tcanvas)
        cv2.waitKey(10)

# for i in range(10):
#     tcanvas = canvas.copy()
#     clocks.draw_digit(tcanvas, i)  
#     cv2.imshow("digit-reset", tcanvas)
#     cv2.waitKey(1000)

cv2.waitKey(0)
cv2.destroyAllWindows()
save_gif(gif_frames, "./clockclock.gif", 50)